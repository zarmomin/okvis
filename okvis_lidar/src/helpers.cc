...
static float deg2rad(const float deg) { return deg * M_PI / 180; }

static float rad2deg(const float rad) { return rad * 180 / M_PI; }
...
void KeyframeFeaturesToCvPoints(const KeyframeFeatures &keyframe,
  ...
  for (std::size_t i = 0u; i < n_keypoints; ++i) {
    keypoints_cv->emplace_back(std::move(
        cv::KeyPoint(keyframe.keypoint_measurements(0, i),              // x
