#ifndef OKVIS_LIDAR_FEATURE_PIPELINE_LK_TRACKING_LASER_H_
#define OKVIS_LIDAR_FEATURE_PIPELINE_LK_TRACKING_LASER_H_
  ...
  processImages2(const aslam::Quaternion &q_Icurr_Iprev,
    ...
    for (size_t frame_idx = 0u; frame_idx < 1u; ++frame_idx) {
      ...
      if (!keypoints_prev.empty()) {
        ...
        const cv::Size kWindowSize = cv::Size(11, 11);
        ...
        for (int i = 0; i < keypoints_curr.size(); ++i) {
          cv::circle(tracker_img, keypoints_curr[i], 3, cv::Scalar(0, 0, 255));
          cv::line(tracker_img, keypoints_prev[i], keypoints_curr[i],
                   cv::Scalar(0, 0, 255));
        }
      }
      ...
      GetElementIndicesInVector(tracking_successful, /*find_value=*/0u,
                                &failed_indices);
      ...
      const bool ransac_success = PerformTemporalFrameToFrameRansac(
          cloud, current_keyframe[frame_idx], previous_keyframe[frame_idx],
          ransac_settings_, q_Icurr_Iprev, &inlier_matches_kp1_k[frame_idx],
          &outlier_matches_kp1_k[frame_idx]);
      ...
      std::transform(outlier_matches_kp1_k[frame_idx].begin(),
                     outlier_matches_kp1_k[frame_idx].end(),
      ...
      // RemoveKeypoints(outlier_indices, &previous_keyframe_features_removed);
      ...
      if (feature_describer_->hasNonDescribableFeatures(
        ...
        RemoveKeypoints(non_describable_indices,
                        &previous_keyframe_features_removed);
      }
      ...
      cv::Mat occupancy_image(curr_camera_images[frame_idx].rows,
                              curr_camera_images[frame_idx].cols, CV_8UC1,
                              cv::Scalar(255));

      std::vector<size_t> keypoint_indices_sorted_by_track_id;
      GetKeypointIndicesSortedByTrackId(current_keyframe[frame_idx],
                                        &keypoint_indices_sorted_by_track_id);
      ...
      for (int kp_idx : keypoint_indices_sorted_by_track_id) {
        ...
        if (occupancy_image.at<uchar>(keypoint) == 255) {
          cv::circle(occupancy_image, keypoint, kMinDistance, cv::Scalar(0),
                     CV_FILLED);
        } else {
      ...
      RemoveKeypoints(converged_keypoint_indices,
                      &previous_keyframe_features_removed);
      ...
      cv::Mat detection_mask(curr_camera_images[frame_idx].rows,
                             curr_camera_images[frame_idx].cols, CV_8UC1,
                             cv::Scalar(255));
      ...
      for (int kp_idx : keypoint_indices_sorted_by_track_id) {
        ...
        if (detection_mask.at<uchar>(keypoint) == 255) {
          cv::circle(detection_mask, keypoint, kRejectFeatureRadiusPx,
      ...
      if (should_redetect) {
        ...
        feature_detector_->detectFeatures(curr_camera_images[frame_idx],
                                          num_to_detect, detection_mask,
                                          &new_detections);
        ...
        CHECK_EQ(new_detections.keypoint_track_ids.cols(),
        ...
        KeyframeFeaturesToCvPoints(new_detections.keypoint_measurements,
                                   &keypoints_curr);
        for (int i = 0; i < keypoints_curr.size(); ++i) {
          cv::circle(tracker_img, keypoints_curr[i], 3, cv::Scalar(255, 0, 0));
        }
      }
      ...
      cv::putText(tracker_img, succ_stream.str(), cv::Point2f(1900, 15),
                  cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 255, 0));
      cv::putText(tracker_img, failed_stream.str(), cv::Point2f(1900, 35),
                  cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255));
      cv::putText(tracker_img, new_stream.str(), cv::Point2f(1900, 55),
      ...
      VLOG(5) << "BEFORE DESCRIBTION: "
              << current_keyframe[frame_idx].keypoint_measurements.cols();
      feature_describer_->describeFeatures(curr_camera_images[frame_idx],
...
#endif // OKVIS_LIDAR_FEATURE_PIPELINE_LK_TRACKING_H_
